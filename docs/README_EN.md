...
| `AGENT_MODE` | Enable Agent skill chat mode (`true`/`false`, default `false`) | Optional |
...
| `AGENT_SKILLS` | Comma-separated active skill ids. Leave empty to use the default-active skill set declared in skill metadata; use `all` to activate every loaded skill. | Optional |
